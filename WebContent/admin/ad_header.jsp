<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
     <link href="http://localhost/sistProject3/common/css/styles.css" rel="stylesheet" />
     
   <nav class="navbar navbar-expand-lg navbar-light bg-light" >
            <div class="container px-4 px-lg-5"  style = "width : 100%">
                <a class="navbar-brand" href="#!" style="font-size: 50px;">ADMIN</a>
                <div  id="loginHeader" style = "width : 50%; text-align : right">
                       <a href ="http://localhost/sistProject3/login/login.jsp">로그인</a> | <a href ="http://localhost/sistProject3/login/terms.jsp">회원가입</a>
                </div>
            </div>
        </nav>
